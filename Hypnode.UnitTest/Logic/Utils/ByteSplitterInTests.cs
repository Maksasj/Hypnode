...
public class ByteSplitterInTests
{
        ...
        LogicValue b3e, LogicValue b2e, LogicValue b1e, LogicValue b0e)
    {
        ...
        var regs = bits.Select(b => { var r = new Register<LogicValue>(); graph.AddNode(r).SetPort(Ports.Input, b); return r; }).ToArray();

        graph.Evaluate();

        LogicValue[] expected = [b0e, b1e, b2e, b3e, b4e, b5e, b6e, b7e];
        for (int i = 0; i < 8; i++)
            Assert.That(regs[i].GetValue(), Is.EqualTo(expected[i]));
    }
}
